...
{
	int i=0,nDeviceId=0,ch_state=0,e_t;
	...
	BS_PHY* pstruBSPhy = (BS_PHY*)DEVICE_PHYVAR(pstruCPEMAC->nBSID,1);
	...
	nDeviceId=fn_NetSim_CR_Channel_Utilization_BS_Subplot(pstruCPEMAC->nBSID);
	if(pstruCPEPHY->pstruOperatingChannel->nChannelState!=ChannelState_PROTECTED)
	{
		ch_state=5;
		sprintf(SU_name[nDeviceId-1][pstruCPEPHY->pstruOperatingChannel->nChannelNumber-1],"SU%d",pstruEventDetails->nDeviceId);
	}
	else
	{
		ch_state=0;
		sprintf(SU_name[nDeviceId-1][pstruCPEPHY->pstruOperatingChannel->nChannelNumber-1],"");
	}
	fn_NetSim_CR_Channel_Utilization_plot_update_SU_yvalue_Update(pstruCPEMAC->nBSID,nDeviceId,pstruCPEPHY->pstruOperatingChannel->nChannelNumber,ch_state);

	fn_NetSim_CR_Channel_Utilization_SU_ystr_Update(pstruCPEMAC->nBSID,nDeviceId);
	sprintf(str,"");
	e_t=pstruEventDetails->dEventTime;
	sprintf(str,"CR_Channel_Utilization(bs_count,%d,SU_xstr%d,SU_ystr%d,SU_yvalue%d,%d,h1,%d)",pstruEventDetails->nDeviceId,nDeviceId,nDeviceId,nDeviceId,plot_flag+1,e_t);
...
{
	int plot_bs_id=0,ch_state=0;
	double e_t=0;
	int nDeviceId=pstruEventDetails->nDeviceId;
	...
	if(pstruGMH->n_UCS == 1 && !pstruBSMAC->nCHSREQFlag)
	{
		...
		//plot
		nDeviceId=fn_NetSim_CR_Channel_Utilization_BS_Subplot(pstruEventDetails->nDeviceId);
		plot_bs_id=fn_NetSim_CR_Channel_Utilization_BS_Subplot(pstruEventDetails->nDeviceId);
		if(pstruBSPHY->pstruOpratingChannel->nChannelState!=ChannelState_PROTECTED)
		{
		ch_state=5;
		//sprintf(SU_name[nDeviceId-1][pstruBSPHY->pstruOpratingChannel->nChannelNumber-1],"SU%d",pstruEventDetails->nDeviceId);
		}
		else
		{
		ch_state=0;
		sprintf(SU_name[nDeviceId-1][pstruBSPHY->pstruOpratingChannel->nChannelNumber-1],"");
		
		}
		fn_NetSim_CR_Channel_Utilization_plot_update_SU_yvalue_Update(pstruEventDetails->nDeviceId,plot_bs_id,pstruBSPHY->pstruOpratingChannel->nChannelNumber,ch_state);
		
		fn_NetSim_CR_Channel_Utilization_SU_ystr_Update(pstruEventDetails->nDeviceId,nDeviceId);
		sprintf(str,"");
		e_t=pstruEventDetails->dEventTime;
		sprintf(str,"CR_Channel_Utilization(bs_count,%d,SU_xstr%d,SU_ystr%d,SU_yvalue%d,%d,h1,%.0lf)",pstruEventDetails->nDeviceId,nDeviceId,nDeviceId,nDeviceId,plot_flag+1,e_t);
		//fn_matlab_execute_command(str);
...
{
	int i=0,nDeviceId=0,ch_state=0;
	double e_t=0;
	BS_MAC* pstruBSMAC = (BS_MAC*)DEVICE_MACVAR(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId);
	BS_PHY* pstruBSPHY = (BS_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId);
	//Initial scan the channel
	...
	//	sprintf(SU_name[nDeviceId-1][pstruBSPHY->pstruOpratingChannel->nChannelNumber-1],"");
	//}
	//fn_NetSim_CR_Channel_Utilization_plot_update_SU_yvalue_Update(pstruEventDetails->nDeviceId,nDeviceId,pstruBSPHY->pstruOpratingChannel->nChannelNumber,ch_state);

	//fn_NetSim_CR_Channel_Utilization_SU_ystr_Update(pstruEventDetails->nDeviceId,nDeviceId);
	//sprintf(str,"");
	//e_t=pstruEventDetails->dEventTime;
	//sprintf(str,"CR_Channel_Utilization(bs_count,%d,SU_xstr%d,SU_ystr%d,SU_yvalue%d,%d,h1,%.0lf)",0,nDeviceId,nDeviceId,nDeviceId,plot_flag+1,e_t);
	//fn_matlab_execute_command(str);
