...
int fn_NetSim_CR_IncumbentStart()
{
	...
	//CR_CHANNEL_UTILIZATION_PLOT
	fn_NetSim_CR_Channel_Utilization_plot_update_PU_yvalue_Update(nDeviceId,nIncumbentId+1,5);
	fn_NetSim_CR_Channel_Utilization_BS_Subplot(nDeviceId);
	nDeviceId=fn_NetSim_CR_Channel_Utilization_BS_Subplot(nDeviceId);
	e_t=pstruEventDetails->dEventTime;
	sprintf(str,"CR_Channel_Utilization(bs_count,%d,In_xstr%d,In_ystr%d,In_yvalue%d,%d,h1,%lf)",nIncumbentId+1,nDeviceId,nDeviceId,nDeviceId,plot_flag,e_t);
...
int fn_NetSim_CR_IncumbentEnd()
{
	...
	//CR_CHANNEL_UTILIZATION_PLOT
	fn_NetSim_CR_Channel_Utilization_plot_update_PU_yvalue_Update(nDeviceId,nIncumbentId+1,0);
	nDeviceId=fn_NetSim_CR_Channel_Utilization_BS_Subplot(nDeviceId);
	e_t=pstruEventDetails->dEventTime;
	sprintf(str,"CR_Channel_Utilization(bs_count,%d,In_xstr%d,In_ystr%d,In_yvalue%d,%d,h1,%lf)",nIncumbentId+1,nDeviceId,nDeviceId,nDeviceId,plot_flag,e_t);
	fn_matlab_execute_command(str);
