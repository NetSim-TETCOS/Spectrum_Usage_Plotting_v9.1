...
_declspec(dllexport) int fn_NetSim_CR_Finish()
{
	...
	fn_matlab_execute_command("close");
	fn_matlab_execute_command("h2=figure('Name','CR- CUMULATIVE CHANNEL UTILIZATION PLOT','Position',[100 100 1000 500],'NumberTitle','off')");
	for(i=1;i<=bs_count;i++)
	{
		//CR_Cumulative_plot(bs_count,flag,xstr,h2) 
		sprintf(str,"CR_Cumulative_plot(%d,%d,In_xstr%d,h2,%.0f)",bs_count,(i*2)-1,i,pstruEventDetails->dEventTime);
	fn_matlab_execute_command(str);
	sprintf(str,"CR_Cumulative_plot(%d,%d,SU_xstr%d,h2,%.0f)",bs_count,(i*2),i,pstruEventDetails->dEventTime);
	fn_matlab_execute_command(str);
	}
	fprintf(stderr,"\nPress any key to terminate MATLAB\n");
	getch();
